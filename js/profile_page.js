...
$(document).ready(function() {
  ...
  function logoutUser() {
    ...
    $editProfileBtn.on('click', function() {
        ...
        let userDetails = JSON.parse(localStorage.getItem('userDetails'));
        if (userDetails) {
            $('#firstName1').val(userDetails.first_name);
            $('#lastName1').val(userDetails.last_name);
            $('#email1').val(userDetails.email);
            $('#phone1').val(userDetails.phone);
        }
        $editProfileModal.fadeIn(500);
    });

    $changePasswordBtn.on('click', function() {
        $changePasswordModal.fadeIn(500);
    });
